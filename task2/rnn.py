...
class RNNModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.batch_first:
            batch_size = x.size(0)
            inputs = x.transpose(0, 1)  # [max_seq_length, batch_size, input_size]
        else:
            batch_size = x.size(1)
            inputs = x
        ...
        for layer in range(self.num_layers):
            ...
            for direction in range(self.num_directions):
                ...
                inputs = inputs if direction == 0 else inputs.flip(0)  # when direction = 1, Ht needs Ht+1
...
def test_GRU_Model():
    ...
    mymodel = RNNModel(input_size, hidden_size, num_layers, bidirectional=bidirectional, batch_first=batch_first,
                       dropout=dropout, mode="GRU")
    for cell in mymodel.cells:
        for w in cell.parameters():
            nn.init.constant_(w.data, 0.01)

    model = nn.GRU(input_size, hidden_size, num_layers, bidirectional=bidirectional, batch_first=batch_first,
                   dropout=dropout)
    for w in model.parameters():
        nn.init.constant_(w.data, 0.01)
