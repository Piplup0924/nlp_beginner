...
class LogisticRegression():
    ...
    def gd(self, x, y):
        ...
        """
        probs = sigmoid(self._exp_dot(x))
        gradients = (np.multiply(x, (y - probs).reshape(-1, 1))).sum(0) / x.shape[0]
    ...
    def mle(self, x, y):
        """Return the MLE estimate, log[p(y|x)]"""
...
class SoftmaxRegression():
    ...
    def predict(self, x):
        """
        Return the predictd classes
        :param x: (batch_size, num_features)
        :return: (batch_size)
        """
        probs = softmax(x.dot(self.w))  # probs: (batch_size, num_classes)
