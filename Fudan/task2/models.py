...
class RNN(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, num_classes, bidirectional=True,
        ...
        # self.rnn = nn.RNN(embed_size, hidden_size, num_layers, bidirectional=bidirectional, batch_first=True)
        self.rnn = RNNModel(embed_size, hidden_size, num_layers, bidirectional=bidirectional, batch_first=True,
                            mode='LSTM')
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, num_classes, num_filters=100, kernel_size=None, dropout_rate=0.3):
        super(CNN, self).__init__()
        if kernel_size is None:
            kernel_size = [3, 4, 5]
        self.embed = nn.Embedding(vocab_size, embed_size)
        self.convs = nn.ModuleList([
            nn.Conv2d(1, num_filters, (k, embed_size), padding=(k - 1, 0)) for k in kernel_size
            # [batch_size, num_filters, seq_length, 1]
        ])
    ...
    def forward(self, x, lens):
        ...
        out = torch.cat(conv_results, 1)  # (batch_size, num_filter * len(kernel_size))
