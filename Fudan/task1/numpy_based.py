...
class Ngram():
    ...
    @staticmethod
    def tokenize(text):
        """In the task, we simply the following tokenizer"""
        return text.lower().split(" ")

    def get_n_grams(self, toks):
        """Get the n-grams word"""
        ngrams_toks = []
        # we can get 2-grams and 3-grams and so on.
...
def get_minibatches(data, minibatch_size, shuffle=True):
    """
    Iterates through the provided data one minibatch at time. You can use this function to
    ...
    Or with multiple data sources:
        for inputs_minibatch, labels_minibatch in get_minibatches([inputs, labels], minibatch_size):
            ...
    :param data: there are two possible values:
    ...
    :return: minibatches: the return value depends on data:
            - If data is a list/array it yields the next minibatch of data.
            - If data a list of lists/arrays it returns the next minibatch of each element in the
              list. This can be used to iterate through multiple data sources
              (e.g., features and labels) at the same time.
    """
    # if we have multiple data source, list_data should be True
...
if __name__ == '__main__':
    ...
    # model = LogisticRegression(X.shape[1], learning_rate, "L2")

    model = SoftmaxRegression(X.shape[1], class_num, learning_rate, regularization, C)

    train_X, dev_X, test_X, train_Y, dev_Y, test_Y = train_test_split(X, Y)

    # Method1: (batch) gradient descent
    # for epoch in range(train_epochs):
    #     train_mle = model.mle(train_X, train_Y)
    #     print("Epoch %s, Train MLE is %.3f" % (epoch, train_mle))
    #     model.gd(train_X, train_Y)
    #     predict_dev_Y = model.predict(dev_X)
    #     print("Epoch %s, Dev Acc %.3f" % (epoch, (predict_dev_Y == dev_Y).sum() / len(dev_Y)))

    # Method2: stochastic gradient descent
    # for epoch in range(train_epochs):
    #     for batch_X, batch_Y in get_minibatches([train_X, train_Y], 1, shuffle=True):
    #         model.gd(batch_X, batch_Y)
    #     predict_dev_Y = model.predict(dev_X)
    #     print("Epoch %s, Dev Acc %.3f" % (epoch, (predict_dev_Y == dev_Y).sum() / len(dev_Y)))

    # Method3: mini-batch gradient descent
    for epoch in range(train_epochs):
        for batch_X, batch_Y in get_minibatches([train_X, train_Y], batch_size, shuffle=True):
            model.gd(batch_X, batch_Y)
        predict_dev_Y = model.predict(dev_X)
        print("Epoch %s, Dev Acc %.3f" % (epoch, (predict_dev_Y == dev_Y).sum() / len(dev_Y)))
    ...
    test['Sentiment'] = model.predict(to_predict_X)
    test[["Sentiment", "PhraseId"]].set_index("PhraseId").to_csv("numpy_based_model.csv")
